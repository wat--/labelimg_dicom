...
# Suppress PyQt5 deprecation warnings for Python 3.10 compatibility
warnings.filterwarnings(
    "ignore", message="sipPyTypeDict.*deprecated", category=DeprecationWarning
)
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(
        ...
        open = action(
            "&Open", self.openFile, "Ctrl+O", "open", "Open image or label file"
        )

        opendir = action(
            "&Open Dir", self.openImagesDirDialog, "Ctrl+u", "open", "Open Dir"
        )

        openDICOMs = action(
            "&Open DICOMs",
            self.openDICOMsDirDialog,
            "Ctrl+P",
            "open",
            "Open DICOM series",
        )
        ...
        openNextImg = action("&Next Image", self.openNextImg, "d", "next", "Open Next")

        openPrevImg = action("&Prev Image", self.openPrevImg, "a", "prev", "Open Prev")

        verify = action(
            "&Verify Image", self.verifyImg, "space", "verify", "Verify Image"
        )
        ...
        close = action(
            "&Close", self.closeFile, "Ctrl+W", "close", "Close current file"
        )

        resetAll = action("&ResetAll", self.resetAll, None, "resetall", "Reset all")
        ...
        remember = action(
            "&Remember\nRectBox",
            self.rememberSelectedShape,
            "Ctrl+c",
            "remember",
            "Remember the selected Box",
            enabled=False,
        )

        recall = action(
            "&Recall\nRectBox",
            self.recallRememberedShape,
            "s",
            "recall",
            "Recall the remembered Box",
            enabled=False,
        )
        ...
        help = action("&Tutorial", self.showTutorialDialog, None, "help", "Show demos")
        showInfo = action(
            "&Information", self.showInfoDialog, None, "help", "Information"
        )
        ...
        self.actions = struct(
            ...
            beginner=(),
            advanced=(),
            editMenu=(edit, copy, delete, remember, recall, None, color1),
        ...
        self.adjustWindowLevelOption.setStatusTip(
            "Adjust window width and level for viewing DICOMs."
        )
        self.adjustWindowLevelOption.triggered.connect(self.adjustWindowLevelDialog)

        # Add auto adjust window/level option
        self.autoAdjustWindowLevelOption = QAction(
            "Auto Adjust DICOM Window/Level", self
        )
        self.autoAdjustWindowLevelOption.setStatusTip(
            "Automatically adjust DICOM window/level based on image content."
        )
        self.autoAdjustWindowLevelOption.triggered.connect(self.autoAdjustDicomWindow)
        ...
        addActions(
            ...
            (
                ...
                self.autoAdjustWindowLevelOption,
                self.autoSaving,
    ...
    def adjustWindowLevelDialog(self):
        # Connect the real-time signal for immediate updates
        self.windowLevelDialog.windowLevelChanged.connect(
            self.onDialogWindowLevelChanged
        )

        # Show the enhanced dialog
        self.windowLevelDialog.popUp(
            w_width=self.settings.get(SETTING_DICOM_WWIDTH, 1000),
            w_level=self.settings.get(SETTING_DICOM_WLEVEL, 200),
        )

    def onDialogWindowLevelChanged(self, width, level):
        """Handle real-time window/level changes from dialog"""
        self.dicomWindowWidth = width
        self.dicomWindowLevel = level

        # Save settings
        self.settings[SETTING_DICOM_WWIDTH] = self.dicomWindowWidth
        self.settings[SETTING_DICOM_WLEVEL] = self.dicomWindowLevel

        # Reload current DICOM image with new settings
        self.reloadCurrentDicomImage()
    ...
    def reloadCurrentDicomImage(self):
        ...
        if self.filePath and DICOMReader.isDICOMFile(self.filePath):
            ...
            if not image.isNull():
                ...
                self.paintCanvas()
                self.statusBar().showMessage(
                    f"DICOM window adjusted: Width={self.dicomWindowWidth}, Level={self.dicomWindowLevel}"
                )

    def autoAdjustDicomWindow(self):
        """Automatically adjust DICOM window/level based on image content."""
        if not (self.filePath and DICOMReader.isDICOMFile(self.filePath)):
            self.statusBar().showMessage("Not a DICOM file")
            return

        try:
            # Read raw DICOM data
            dcm = DICOMReader.readRawDICOM(self.filePath)
            pixels = DICOMReader._dicomToRaw(dcm)
            ...
            # Set level to middle of range, width to full range
            auto_level = int((p1 + p99) / 2)
            auto_width = int(p99 - p1)

            # Apply limits to prevent extreme values
            auto_width = max(1, min(auto_width, 20000))  # Width between 1 and 20000
            auto_level = max(
                -10000, min(auto_level, 10000)
            )  # Level between -10000 and 10000

            self.dicomWindowWidth = auto_width
            self.dicomWindowLevel = auto_level

            # Save settings
            self.settings[SETTING_DICOM_WWIDTH] = self.dicomWindowWidth
            self.settings[SETTING_DICOM_WLEVEL] = self.dicomWindowLevel

            # Reload image with auto-adjusted settings
            self.reloadCurrentDicomImage()

        except Exception as e:
            self.statusBar().showMessage(f"Auto-adjust failed: {str(e)}")
        ...
        try:
            if self.usingPascalVocFormat is True:
                ...
                self.labelFile.savePascalVocFormat(
                    annotationFilePath, shapes, self.filePath, self.imageShape
                )
    ...
    def loadFile(self, filePath=None):
        ...
        if unicodeFilePath and os.path.exists(unicodeFilePath):
            if LabelFile.isLabelFile(unicodeFilePath):
                ...
                # Auto-adjust DICOM window/level on initial load
                try:
                    # Read raw DICOM data for auto-adjustment
                    dcm = DICOMReader.readRawDICOM(unicodeFilePath)
                    pixels = DICOMReader._dicomToRaw(dcm)
                    ...
                    # Apply limits to prevent extreme values
                    auto_width = max(1, min(auto_width, 20000))
                    auto_level = max(-10000, min(auto_level, 10000))

                    # Update current values with auto-adjusted ones
                    self.dicomWindowWidth = auto_width
                    self.dicomWindowLevel = auto_level
                    ...
                    self.statusBar().showMessage(
                        f"DICOM auto-adjusted: Width={self.dicomWindowWidth}, Level={self.dicomWindowLevel}"
                    )
                except Exception as e:
                    # If auto-adjust fails, use default values
                    self.statusBar().showMessage(
                        f"Auto-adjust failed, using defaults: {str(e)}"
                    )
    ...
    def importDirDICOMs(self, dirpath):
        ...
        self.dicomDialog = DICOMDialog(
            parent=self, listItem=[s.to_str() for s in series_infos]
        )
    ...
    def refreshImg(self):
        ...
        if self.filePath is None:
            ...
            try:
                ...
                # Current file path is not in the image list, just use the first image if available
                if len(self.mImgList) > 0:
                    filename = self.mImgList[0]
